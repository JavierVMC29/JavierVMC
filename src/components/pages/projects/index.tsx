...
const ProjectsPage = () => {
  ...
  const getProjectTags = (key: string, tagAmount: number, tags: string[]) => {
    ...
    Array.from({ length: tagAmount }).map((_, index) => projectTags.push(`${t(`${key}.tag_${index + 1}`)}`));
    ...
    <main className="page-container">
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8 max-w-[1440px]">
        {Object.entries(PROJECTS_ITEMS).map(([key, { image, href, githubHref, tags }], index) => (
          <div key={key}>
            <ProjectsItem image={image} title={t(`${key}.title`)} description={t(`${key}.description`)} tags={getProjectTags(key, projectTags[index], tags)} href={href} githubHref={githubHref} />
          </div>
        ))}
      </section>
    </main>
